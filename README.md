...
**Input architecture:** Inputs are co-located with the code blocks that use them. `submodel.inputs` holds experimental conditions for ODE code, `observable.inputs` holds values for observable code, and `empirical_data.inputs` holds values for distribution derivation. State variables include their initial values directly (no reference indirection). Inputs support figure-extracted data via `source_type` field (`text`, `table`, or `figure`) with required `figure_id` and `extraction_method` for figure sources.
