...
extraction and test statistic generation, excluding footer fields that are
...
from maple.core.calibration.shared_models import WeightScore
...
# ============================================================================
# Footer Models (Metadata about files, not generated by LLM)
# ============================================================================


class ParameterFooters(BaseModel):
    """
    Footer fields for parameter metadata files.
...
class ParameterMetadata(BaseModel):
    ...
    Footers are stripped before sending to LLM, then added back during unpacking.
    """
    ...
    def get_header_fields(cls) -> set[str]:
        """Get set of field names that are headers (from ParameterFooters)."""
        return set(ParameterFooters.model_fields.keys())

    def split(self) -> tuple[ParameterFooters, dict]:
        """
        ...
        headers = ParameterFooters(**header_data)
    ...
    @classmethod
    def from_split(cls, headers: ParameterFooters, content: dict) -> "ParameterMetadata":
        """
...
class TestStatisticFooters(BaseModel):
    """
    Footer fields for test statistic files.
...
class TestStatistic(BaseModel):
    ...
    Excludes footer fields that are added during unpacking:
    - test_statistic_id, cancer_type
    ...
    @classmethod
    def get_header_fields(cls) -> set[str]:
        """Get set of field names that are headers (from TestStatisticFooters)."""
        return set(TestStatisticFooters.model_fields.keys())

    def split(self) -> tuple[TestStatisticFooters, dict]:
        """
        Split into headers and content.
        ...
        # Extract headers
        header_data = {k: v for k, v in all_data.items() if k in header_fields}
        headers = TestStatisticFooters(**header_data)
    ...
    @classmethod
    def from_split(cls, headers: TestStatisticFooters, content: dict) -> "TestStatistic":
        """
        Create TestStatistic from headers and content.

        Args:
            headers: TestStatisticFooters instance
            content: Content dictionary (LLM-generated fields)
...
class ModelRegistry:
    ...
    _registry: dict[type[BaseModel], type[BaseModel]] = {
        ParameterMetadata: ParameterFooters,
        TestStatistic: TestStatisticFooters,
        CalibrationTarget: CalibrationTargetFooters,
    }
    ...
    def get_header_model(cls, model_class: type[BaseModel]) -> type[BaseModel]:
        ...
        Returns:
            Header model class (e.g., ParameterFooters)
