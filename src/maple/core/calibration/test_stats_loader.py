...
Compiles calibration target YAML files (maple's own schema) into the
schema-agnostic test-stats DataFrame consumed by the qsp-hpc-tools runtime
(``build_test_stat_registry`` / ``compute_test_statistics_batch``). This
compilation lives in maple so the runtime never has to parse maple's schema;
it receives an already-compiled DataFrame from its caller. Each YAML
