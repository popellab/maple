...
- **Optional by default; REQUIRED once a target has 2+ population-spread observables.** A single population-spread observable (or observables that don't feed the spread — `center_only` / `technical`) needs no tag; each is its own group. This covers the large majority of targets. But as soon as a target carries TWO OR MORE observables with a population `spread_source` (`across_patient` / `biological_experimental`), you MUST tag every one of them — the validator rejects an untagged population-spread entry in that case. Leaving them untagged silently treats possibly-shared-unit measurements as independent and shrinks the spread by ~sqrt(K). Same string = same units; different strings = separate populations.
- Share a group ONLY across observables from the SAME population of biological units. Members of one group MUST agree on `spread_source` and `experimental_unit_type` (they describe the shared population and unit type) — the validator enforces this. `n_biological` MAY differ across members: an unbalanced panel (a donor measured at some points but not all, so per-point n differs) is still one population viewed several times, and the per-point n feeds the consumer's moment weighting rather than defining the batch.
