...
# ============================================================================
# Isolated System Target
# ============================================================================

# Import here to avoid circular imports
# CalibrationTarget is imported at runtime when IsolatedSystemTarget is instantiated


def _get_calibration_target():
    """Lazy import of CalibrationTarget to avoid circular imports."""
    from qsp_llm_workflows.core.calibration_target_models import CalibrationTarget

    return CalibrationTarget


class IsolatedSystemTarget(BaseModel):
    """
    A calibration target from an isolated/in vitro experimental system.

    Extends CalibrationTarget fields and adds:
    ...
    Note: This class duplicates CalibrationTarget fields rather than using inheritance
    to avoid circular import issues. Both classes share the same field structure.
    """

    # Import CalibrationTarget fields dynamically to share field definitions
    # For now, we define the key additional field and rely on composition
    ...
    class Config:
        ...
        extra = "allow"  # Allow extra fields from CalibrationTarget


# Create the actual inheriting class after imports are resolved
# This is a factory function that creates the proper inheriting class
def create_isolated_system_target_class():
    """
    Create IsolatedSystemTarget class that properly inherits from CalibrationTarget.

    Call this after all imports are resolved to get the full class with inheritance.
    """
    from qsp_llm_workflows.core.calibration_target_models import CalibrationTarget

    class IsolatedSystemTargetFull(CalibrationTarget):
        """
        A calibration target from an isolated/in vitro experimental system.
        ...
        def at_least_one_cut(cls, v: List[Cut]) -> List[Cut]:
            """Ensure at least one cut is provided."""
            if len(v) < 1:
                raise ValueError("At least one cut is required to define the isolated system.")
            return v

        @model_validator(mode="after")
        def validate_no_duplicate_cuts(self) -> "IsolatedSystemTargetFull":
            """Ensure no duplicate cuts (same type + name)."""
            seen = set()
            for cut in self.cuts:
                key = (cut.type, cut.name)
        ...
        @model_validator(mode="after")
        def validate_in_vitro_system(self) -> "IsolatedSystemTargetFull":
    ...
    return IsolatedSystemTargetFull
