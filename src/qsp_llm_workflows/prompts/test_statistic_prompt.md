...
**NOTE:** The `model_output_code` is provided by the user during CSV preparation and is NOT generated by the LLM. The user writes the `compute_test_statistic` function with Pint unit handling, and it is validated during enrichment.

See the user-provided `model_output_code` in the input data - you do NOT need to generate this code.
