...
def test_model_exporter_has_export_to_json_method():
    ...
    try:
        ...
        # Check it's callable
        assert callable(exporter.export_to_json), \
            "export_to_json is not callable"
...
def test_model_exporter_has_export_definitions_method():
    ...
    try:
        ...
        assert hasattr(exporter, "export_definitions"), \
            "ModelDefinitionExporter is missing export_definitions method"
        assert callable(exporter.export_definitions), \
            "export_definitions is not callable"
...
def test_model_exporter_finds_matlab_export_script():
    ...
    try:
        ...
        assert expected_script.exists(), \
            f"MATLAB export script not found at {expected_script}"
